...
class DataCollatorForLanguageModeling:
    ...
    def __call__(
        ...
        if isinstance(examples[0], (dict, BatchEncoding)):
            ...
            examples_Ngram_attention_mask = np.array([e["Ngram_attention_mask"] for e in examples])  #Ngram_attention_mask is list of ndarray, each is size 128
            ...
            examples_Ngram_position_matrix = np.array([e["Ngram_position_matrix"] for e in examples])  #Ngram_position_matrix is a list of ndarray, each is (128, 128)
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Kyle fix: convert list to torch.tensor
        
        inputs[indices_replaced] = torch.tensor(self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token), dtype=torch.long)
...
class LineByLineTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, Ngram_dict, file_path: str, block_size: int, model_type: str):
        max_length = block_size # (256  #add by @shizhe) updated to blocksize by Kyle
        ...
        for i in range(len(examples)):
            ...
            for p in range(2, 8):
                for q in range(0, len(tokens) - p + 1):
                    ...
                    # xlm-roberta uses underscores
                    character_segment = tmp_text.replace('Ġ', ' ').strip() if model_type == "roberta" else tmp_text.replace('_', ' ').strip()

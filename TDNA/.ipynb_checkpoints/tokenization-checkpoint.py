...
# Kyle fix: AddedToken was missing
...
class PreTrainedTokenizer(PreTrainedTokenizerBase):
    ...
    def convert_tokens_to_ids(self, tokens: Union[str, List[str]]) -> Union[int, List[int]]:
        ...
        # Kyle fix: when tokens is not a list but AddedToken
        # for some reason we get the <mask> token here. not sure why that's happening.
        
        if type(tokens) == AddedToken:
            ids.append(self._convert_token_to_id_with_added_voc(tokens.content))
        else:
            for token in tokens:

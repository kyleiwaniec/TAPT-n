...
parser.add_argument('--LLMvocab_path', type=str, required=True, help="path to LLM vocab file (vocab.json)")
...
LLMvocab_path = config.LLMvocab_path
...
t = tqdm(ngram_finder.ngrams.items())
count = 0
for w, c in t:
...
with open(LLMvocab_path) as json_file:
    LLMvocab = json.load(json_file)
    
LLMvocab_list = [s.replace('Ġ','') for s in list(LLMvocab.keys())]
    
common_vocab = set(ngram_list).intersection(set(LLMvocab_list))
    
# remove words which are already in the LLM vocabulary
for w in list(common_vocab):
    # remove w from ngram_list
    ngram_list.remove(w)

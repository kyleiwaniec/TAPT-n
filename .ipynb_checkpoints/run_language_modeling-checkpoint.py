...
from tokenization import RobertaTokenizer, PreTrainedTokenizer, XLMRobertaTokenizer
...
def get_dataset(
    ...
    return LineByLineTextDataset(tokenizer=tokenizer, Ngram_dict=Ngram_dict, file_path=file_path,
                                 block_size=args.block_size, model_type=args.model_type)
...
def main():
    ...
    parser.add_argument("--add_tokens",
                        default=False,
                        type=bool,
                        required=False,
                        help="Whether to add the ngram tokens to vocab")
    ...
    parser.add_argument("--max_position_embeddings",
                        ...
                        type=int,
                        required=False,
                        help="Maximum length if input sequence.")
    ...
    if args.add_tokens:
        ...
        new_tokens = []
        with open(args.Ngram_path) as tokens_file:
            for line in tokens_file:
                token,count = line.strip().split('\t')
                new_tokens.append(token)
        tokenizer.add_tokens(new_tokens)
        
        # The new (randomly initialized) vector is added at the end of the embedding matrix
        model.resize_token_embeddings(len(tokenizer)) 
#         tokenizer.unique_no_split_tokens = {token: None for token in tokenizer.unique_no_split_tokens}

        # intialize the added tokens with the ngram embeddings. 
        fasttext_ngrams = np.load(args.fasttext_model_path)
        model.roberta.embeddings.word_embeddings.weight.data[50265:, :] = torch.from_numpy(fasttext_ngrams)
    ...
    '''
    prior to adding ngrams: len(tokenizer) 50265
    after addign the ngrams: len(tokenizer)2 83033
    '''    
...
#     print("summary ",torch.cuda.memory_summary(device=None, abbreviated=False))
    ...
    if args.do_train:
        ...
        if trainer.is_world_process_zero():
            # make sure to save the added_tokens for inference time!
            added_toks = tokenizer.get_added_vocab()
            with open(args.output_dir+'/added_tokens.json','w') as _f:
                json.dump(added_toks, _f)
    ...
    if args.do_eval:
        ...
        perplexity = math.exp(eval_output.metrics["eval_loss"])
